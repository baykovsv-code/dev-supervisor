...
Status: **historical completed 2.0 input, retained temporarily for the T20
neutralization transition**. This document normalized the backlog into stable
identifiers. It does not authorize current implementation by itself.
...
| R17 | Rolling legacy-reference compatibility | Every implementation candidate through T11 in authoritative plan order, including inserted prerequisites, must pass a no-model compatibility harness against a redacted real T30 policy/state fixture. The legacy reference project remains on an immutable AS-IS checkout until T12 and the final human cutover gate. |
...
8. A ticket is not complete if the candidate cannot safely inspect and reconcile the
   approved legacy reference fixture without a model or state mutation.
...
- no macOS support claim in the initial 2.0 cutover. Restricted-workplace macOS
  qualification is a [2.1 backlog candidate](../backlog/macos-platform-qualification-2.1.md).
